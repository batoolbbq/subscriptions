...
@section('content')
    ...
    --brown:#8C5346;--green-50:#e9fbf2;--green-700:#10734a;--blue:#2563eb;--orange:#ea580c;">
        ...
        @if ($phone && !$agent)
            <div class="alert alert-danger" style="border-radius:14px">🚫 لم يتم العثور على وكيل بهذا الرقم:
                {{ $phone }}</div>
        @endif
        ...
        @if ($agent)
            {{-- بطاقة بيانات الوكيل --}}
            <div
                style="border:1.5px solid var(--line);border-radius:20px;overflow:hidden;box-shadow:0 10px 28px rgba(0,0,0,.06);background:#fff;margin-bottom:20px;">
                <div
            ...
            {{-- كارت إجمالي الخدمات (شكل ولون جديد) --}}
            <div class="row g-3 mb-4">
                <div class="col-md-6">
                    <div class="h-100"
                        style="border-radius:18px;background:linear-gradient(135deg,#2563eb,#3b82f6);
                           box-shadow:0 10px 24px rgba(37,99,235,.25);color:#fff;">
            ...
            {{-- أزرار عرض الجداول (ستايل جديد) --}}
            <div class="row g-3 mb-2">
                <div class="col-md-6">
                    <a class="btn w-100" href="{{ route('agents.services.customers', $agent->id) }}"
                        style="border-radius:12px;background:#2563eb;color:#fff;font-weight:800;
                          padding:10px 14px;box-shadow:0 8px 20px rgba(37,99,235,.25);">
                        📋 عرض خدمات المشتركين
                    </a>
                </div>
                <div class="col-md-6">
                    <a class="btn w-100" href="{{ route('agents.services.institutions', $agent->id) }}"
                        style="border-radius:12px;background:#ea580c;color:#fff;font-weight:800;
                          padding:10px 14px;box-shadow:0 8px 20px rgba(234,88,12,.25);">
