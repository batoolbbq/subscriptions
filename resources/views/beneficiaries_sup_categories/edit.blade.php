...
<div class="container py-4" style="--ink:#111827;--line:#e5e7eb;--blue-50:#f3f6ff;--blue-200:#cfd8ff;--blue-700:#1d4ed8;--amber-50:#fff5e6;--amber-200:#ffd8a8;--amber-800:#92400e;--red-50:#fff1f1;--red-200:#ffc9c9;--red-700:#b42318;--green-50:#e9fbf2;--green-700:#10734a;--gray-50:#eff2f6;--gray-700:#374151;">
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
    <div>
      <h3 style="margin:0;font-weight:800;color:var(--ink);">تعديل فئة فرعية</h3>
      <div style="color:#6b7280;font-size:14px;">قم بتعديل بيانات الفئة الفرعية ثم اضغط على زر التحديث للحفظ.</div>
    </div>
  ...
  {{-- الفورم --}}
  <form method="POST" action="{{ route('beneficiaries-sup-categories.update', $item->id) }}">
    @csrf
    @method('PUT')
    ...
    <div style="border:1.5px solid var(--line);border-radius:14px;box-shadow:0 8px 20px rgba(17,24,39,.04);margin-bottom:16px;overflow:hidden;">
      ...
      <div style="padding:16px;">
        <div class="mb-3">
          ...
          <input type="text" name="name" class="form-control" style="border:1.5px solid #E5E7EB;" value="{{ old('name', $item->name) }}" maxlength="255" required>
        </div>

        <div class="mb-3">
          <label style="font-weight:700;color:var(--gray-700);">النوع <span style="color:red;">*</span></label>
          <input type="text" name="type" class="form-control" style="border:1.5px solid #E5E7EB;" value="{{ old('type', $item->type) }}" maxlength="100" required>
        </div>

        <div class="mb-3">
          <label style="font-weight:700;color:var(--gray-700);">الكود (أرقام فقط حتى 5) <span style="color:red;">*</span></label>
          <input type="text" name="code" class="form-control" style="border:1.5px solid #E5E7EB;" maxlength="5" value="{{ old('code', $item->code) }}" onkeypress="return event.charCode >= 48 && event.charCode <= 57" required>
        </div>
        ...
        <div class="mb-3">
          ...
          <select name="beneficiaries_categories_id" class="form-control" style="border:1.5px solid #E5E7EB;" required>
            ...
            @foreach($mainCategories as $cat)
              <option value="{{ $cat->id }}" {{ old('beneficiaries_categories_id', $item->beneficiaries_categories_id) == $cat->id ? 'selected' : '' }}>
                {{ $cat->name }}
              </option>
            @endforeach
          </select>
        ...
        <div class="mb-3">
          ...
          <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="status" id="status_active" value="1" {{ old('status', $item->status) == 1 ? 'checked' : '' }}>
            <label class="form-check-label" for="status_active">مفعّلة</label>
          </div>
          <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="status" id="status_inactive" value="0" {{ old('status', $item->status) == 0 ? 'checked' : '' }}>
